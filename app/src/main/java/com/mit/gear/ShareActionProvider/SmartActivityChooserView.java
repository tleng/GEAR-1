 ...
 * @hide
 */
class SmartActivityChooserView extends ViewGroup
		implements SmartActivityChooserModel.ActivityChooserModelClient{
  private final ActivityChooserViewAdapter mAdapter;				//An adapter for displaying the activities in an {@link AdapterView}.
  private final Callbacks mCallbacks;								//Implementation of various interfaces to avoid publishing them in the APIs.
  private final LinearLayoutCompat mActivityChooserContent;			//The content of this view.
  private final Drawable mActivityChooserContentBackground;			//Stores the background drawable to allow hiding and latter showing.
  private final FrameLayout mExpandActivityOverflowButton;			//The expand activities action button;
  private final ImageView mExpandActivityOverflowButtonImage;		//The image for the expand activities action button;
  private final FrameLayout mDefaultActivityButton;					//The default activities action button;
  private final ImageView mDefaultActivityButtonImage;				//The image for the default activities action button;
  private final int mListPopupMaxWidth;								//The maximal width of the list popup.
  ActionProvider mProvider;											//The ActionProvider hosting this view, if applicable.
   ...
   *
   */
  private ListPopupWindow mListPopupWindow;						//Popup window for showing the activity overflow list.
  private PopupWindow.OnDismissListener mOnDismissListener;		//Listener for the dismissal of the popup/alert.
  private boolean mIsSelectingDefaultActivity;					//Flag whether a default activity currently being selected.
  private int mInitialActivityCount = ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_DEFAULT;		//The count of activities in the popup.
  private boolean mIsAttachedToWindow;							//Flag whether this view is attached to a window.
  private int mDefaultActionButtonContentDescription;			//String resource for formatting content description of the default target.

  /**
   * Create a new instance.
   * @param context The application environment.
   */
  public SmartActivityChooserView(Context context)
  {
    this(context, null);
  }
   ...
   */
  public SmartActivityChooserView(Context context, AttributeSet attrs)
  {
  ...
  public SmartActivityChooserView(Context context, AttributeSet attrs, int defStyle){
    ...
    TypedArray attributesArray = context.obtainStyledAttributes
			(attrs, R.styleable.ActivityChooserView, defStyle, 0);
   ...
   * overflow list. Clients would like to set this drawable
  ...
  public SmartActivityChooserModel getDataModel()
  {
  ...
  private ListPopupWindow getListPopupWindow(){
    if (mListPopupWindow == null){
      ...
      mListPopupWindow.setAnchorView(SmartActivityChooserView.this);
  ...
  private void updateAppearance(){
    ...
    //Set if (false) so the history icon will not be shown
    if (false){
   ...
   */
  private class Callbacks implements AdapterView.OnItemClickListener, OnClickListener,
		  OnLongClickListener, PopupWindow.OnDismissListener {
  ...
  private class ActivityChooserViewAdapter extends BaseAdapter{
    ...
    private SmartActivityChooserModel mDataModel;
    private int mMaxActivityCount = MAX_ACTIVITY_COUNT_DEFAULT;
    private boolean mShowDefaultActivity;
    private boolean mHighlightDefaultActivity;
    private boolean mShowFooterView;
    public void setDataModel(SmartActivityChooserModel dataModel){
      SmartActivityChooserModel oldDataModel = mAdapter.getDataModel();
      ...
      case ITEM_VIEW_TYPE_FOOTER:
        if (convertView == null || convertView.getId() != ITEM_VIEW_TYPE_FOOTER) {
          convertView = LayoutInflater.from(getContext())
				  .inflate(R.layout.abc_activity_chooser_view_list_item, parent, false);
